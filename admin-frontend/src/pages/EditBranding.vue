  ...
  <div id="content" class="app-content">
    ...
    <b-form @submit.prevent="editBranding" v-if="!loading && !notFound">
      <b-card>
        <b-row>
          <b-col cols="12">
            <label for>Upload Company Logo/Professional Headshot</label>
            <div
              class="image"
              @click="pickFile"
              v-bind:style="{
                'background-image': 'url(' + getImage(branding.image) + ')',
              }"
            >
      ...
      <b-card class="mt-2">
        <b-row>
          <b-col cols="12" md="6" lg="4" class="mt-2">
            ...
            <b-collapse id="collapse-1" :visible="true" class="pt-2">
              <b-form-group>
                <label>Service</label>
                <b-form-select
      ...
      <b-row>
        <b-col cols="12" class="text-right mt-3">
          <b-button type="submit" variant="primary" :disabled="editLoading">{{
  ...
  methods: {
    ...
    async editBranding() {
      ...
      if (!this.branding.start_date) {
        this.toast("Please Select Date Range", "Add Branding", "warning");
        return;
      }
      if (
        this.branding.image === "null" ||
        this.branding.image === "undefined"
      ) {
        delete this.branding.image;
      }
    ...
    setBranding(val) {
      ...
      if (this.branding.image == null) {
        console.log(this.branding.image);
        delete this.branding.image;
      }
      //   if (
      //     this.branding.image === "null" ||
